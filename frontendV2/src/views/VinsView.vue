<template>
  <div class="p-4">
  <h1 class="text-2xl font-bold text-gray-900 mb-4">Liste des vins</h1>
  <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
  <div v-for="vin in vins" :key="vin._id" class="bg-white shadow-lg overflow-hidden">
    <!-- Image du vin -->
    <img :src="vin.fileUrl" alt="Bottle of Wine" class="w-full h-40 object-contain">

    <div class="p-5">
      <h3 class="text-lg font-bold text-gray-900">{{ vin.nom }}</h3>
      <p class="text-gray-600">{{ vin.region }} - {{ vin.cepage }}</p>
      <p class="text-gray-600">Millésime: {{ vin.annee }} | Quantité: {{ vin.quantite }}</p>
  ...
  <div v-if="editFormVisible" class="mt-4 p-4 border border-gray-200 rounded">
    <h2 class="text-xl font-semibold text-gray-800 mb-3">Modifier vin</h2>
    <form @submit.prevent="updateVin" class="space-y-3">
      ...
      <input class="p-2 border border-gray-300 w-full" type="number" v-model.number="editableVin.annee" placeholder="Millésime">
      ...
      <button class="px-4 py-2 bg-green-500 text-white hover:bg-green-600" type="submit">Modifier Vin</button>
      <button class="px-4 py-2 bg-gray-500 text-white hover:bg-gray-600" @click="editFormVisible = false">Annuler</button>
    </form>
  </div>

  <h2 class="text-xl font-semibold text-gray-800 mt-6 mb-3">Ajouter un vin</h2>
  <form @submit.prevent="submitVin" class="space-y-3">
    ...
    <input class="p-2 border border-gray-300 w-full" type="number" v-model.number="newVin.annee" placeholder="Millésime">
    ...
    <button class="px-4 py-2 bg-blue-500 text-white hover:bg-blue-600" type="submit">Ajouter</button>
  </form>
</div>

<template>
    <div id="signupBg">
        <button @click="$emit('close-signup')">close</button>
        <form name="signup-form">
            <div class="mb-3 center">
                <input id="email" class="logInputs" type="email" placeholder="Email" v-model="input.email" />
            </div>
            <div class="mb-3 center">
                <input id="username" class="logInputs" type="text" placeholder="Username" v-model="input.email" />
            </div>
            <div class="mb-3 center">
            ...
            <br>
            <button class="signupBtn" type="submit" v-on:click.prevent="signup()">
                Signup
            </button>
        ...
        signup() {
            if (this.input.email !== "" && this.input.password !== "") {
                axios.post('http://localhost:3000/api/auth/signup', {
                    ...
                    .catch(error => {
                        console.error("Signup failed", error.response ? error.response.data : 'Error without response');
